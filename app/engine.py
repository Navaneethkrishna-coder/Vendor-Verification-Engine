...
async def run_verification_engine(
    ...
    """
    Executes the 7-stage vendor onboarding verification pipeline in exact order.
    ...
    if missing_fields or invalid_format_fields:
        ...
        s1_summary = f"Validation failed: {len(issues)} missing or invalid fields."
        s1_risk = 40
        s1_flag = "missing_fields"
        context["missing_items"] = [f"Field error: {item}" for item in issues]
    else:
        s1_status = StageStatus.PASS
        s1_summary = "All required submission fields are present and well-formed."
        s1_risk = 0
        s1_flag = None
    ...
    stage1 = StageResult(
        ...
        risk_points=s1_risk,
        flag_type=s1_flag,
        notes=[] if s1_status == StageStatus.PASS else [f"Issues identified: {', '.join(issues)}"]
    )
    stages.append(stage1)
    accumulated_risk += s1_risk
    yield {"event": "stage_update", "stage": stage1.model_dump(), "progress": 14}
    ...
    if unreadable_docs:
        ...
        s2_summary = f"Document validation failed: {len(unreadable_docs)} document(s) missing or unreadable."
        s2_risk = 40
        s2_flag = "unreadable_documents"
        ...
        s2_summary = "All 3 required PDF documents are present and text is extractable."
        s2_risk = 0
        s2_flag = None
    ...
    stage2 = StageResult(
        ...
        risk_points=s2_risk,
        flag_type=s2_flag,
        notes=[] if s2_status == StageStatus.PASS else [f"Document errors: {', '.join(unreadable_docs)}"]
    )
    stages.append(stage2)
    accumulated_risk += s2_risk
    yield {"event": "stage_update", "stage": stage2.model_dump(), "progress": 28}

    # -------------------------------------------------------------
    # STAGE 3: Field Extraction from Documents
    # -------------------------------------------------------------
    ...
    if missing_extractions:
        s3_status = StageStatus.WARNING
        s3_summary = f"Partial extraction: Missing {len(missing_extractions)} critical label(s)."
        s3_risk = 15
        s3_flag = "partial_extraction"
    else:
        s3_status = StageStatus.PASS
        s3_summary = f"Successfully parsed label:value pairs across all 3 documents ({', '.join(extraction_summary_items)})."
        s3_risk = 0
        s3_flag = None
    ...
    yield {"event": "stage_update", "stage": stage3.model_dump(), "progress": 42}
    ...
    # Determine Stage 4 status & risk points
    if critical_wrong_company_docs:
        s4_status = StageStatus.FAIL
        s4_summary = f"CRITICAL: Uploaded identity document(s) issued to a completely different company ({critical_wrong_company_docs[0]['document']}: '{critical_wrong_company_docs[0]['doc_value']}')."
        s4_risk = 85
        s4_flag = "critical_wrong_company_doc"
    elif field_discrepancies:
        s4_status = StageStatus.WARNING
        s4_summary = f"Field consistency check flagged {len(field_discrepancies)} discrepancy item(s) between form and documents."
        s4_risk = 45
        s4_flag = "field_mismatch"
    elif bank_holder_explained:
        s4_status = StageStatus.INFO
        s4_summary = f"All fields match. Bank account name variation cleared by vendor relationship explanation note."
        s4_risk = 10
        s4_flag = "name_mismatch_explained"
    else:
        s4_status = StageStatus.PASS
        s4_summary = "All 10 extracted document fields match the submitted form values exactly."
        s4_risk = 0
        s4_flag = None
    ...
    yield {"event": "stage_update", "stage": stage4.model_dump(), "progress": 57}

    # -------------------------------------------------------------
    # STAGE 5: Tax ID & Bank Country Consistency
    # -------------------------------------------------------------
    s5_notes: List[str] = []
    s5_risk = 0
    s5_flag = None
    ...
    if not tax_format_valid_for_declared and detected_other_country and detected_other_country != declared_country:
        ...
        s5_summary = f"Critical Tax ID anomaly: Declared '{declared_country}', but Tax ID '{tax_id_to_check}' matches format for '{detected_other_country}'."
        s5_notes.append(f"Hard inconsistency: Pattern matches {TAX_ID_PATTERNS[detected_other_country]['description']} rather than {declared_country}")
        s5_risk = 85
        s5_flag = "tax_country_fraud"
        context["tax_detected_country"] = detected_other_country
    elif not tax_format_valid_for_declared:
        s5_status = StageStatus.WARNING
        s5_summary = f"Tax ID '{tax_id_to_check}' does not match standard pattern for {declared_country}."
        s5_notes.append(f"Expected format pattern for {declared_country}")
        s5_risk = 25
        s5_flag = "tax_format_irregular"
    else:
        s5_status = StageStatus.PASS
        s5_summary = f"Tax ID format matches expected standard for {declared_country}."
    ...
    if len(bank_acc) >= 2 and bank_acc[:2].isalpha():
        ...
        if expected_iso and iban_country_code != expected_iso:
            s5_notes.append(f"Cross-border banking note: IBAN country prefix '{iban_country_code}' differs from declared country '{declared_country}' (non-blocking)")
            s5_risk += 5
    ...
    yield {"event": "stage_update", "stage": stage5.model_dump(), "progress": 71}

    # -------------------------------------------------------------
    # STAGE 6: Risk & Vendor History Check
    # -------------------------------------------------------------
    s6_notes: List[str] = []
    s6_risk = 0
    s6_flag = None
    ...
    for rec in bank_history:
        ...
        if sim < 0.85:
            ...
            s6_notes.append(f"CRITICAL FRAUD SIGNAL: Bank account {submission.bank_account_number} is already on file for '{conflict_company}' (Run: {rec['run_id']})")
    ...
    if approved_past_records:
        ...
        s6_notes.append(f"Recognized Returning Vendor: Prior approval on file (Run: {approved_past_records[0]['run_id']})")
    ...
    if valid_until_str:
        ...
        if parsed_valid_until:
            ...
            if parsed_valid_until < today_date:
                ...
                s6_notes.append(f"Tax certificate expired on {valid_until_str} (Prior to today {today_date.isoformat()})")
    ...
    if email_domain in FREE_EMAIL_DOMAINS:
        s6_notes.append(f"Soft signal: Personal/free email domain detected (@{email_domain}). Non-blocking.")
        s6_risk += 10

    if is_duplicate_bank_reuse:
        s6_status = StageStatus.FAIL
        s6_summary = f"CRITICAL: Bank account reuse detected across different legal entities ('{conflict_company}')."
        s6_risk = 100
        s6_flag = "duplicate_bank_account"
    elif is_returning_vendor and is_tax_cert_expired:
        s6_status = StageStatus.WARNING
        s6_summary = f"Returning vendor verified on file; submitted tax certificate expired on {expired_date_str}."
        s6_risk = 25
        s6_flag = "returning_vendor_expired_doc"
    elif is_returning_vendor:
        s6_status = StageStatus.PASS
        s6_summary = "Returning vendor verified with prior clean approval in vendor ledger."
    else:
        s6_status = StageStatus.PASS
        s6_summary = "No conflicting bank records or adverse history found in vendor ledger."
    ...
    yield {"event": "stage_update", "stage": stage6.model_dump(), "progress": 85}

    # -------------------------------------------------------------
    # STAGE 7: Decision Priority & Risk Synthesis
    # -------------------------------------------------------------
    # ORDER OF PRECEDENCE (Strict Decision Priority Order):
    # 1. Duplicate bank account reuse under a different company name -> Rejected (Escalate to compliance)
    # 2. Tax ID format provably matches a different country than declared -> Rejected
    # 3. Registration or Tax cert issued to completely different company (< 55% similarity) -> Rejected
    # 4. Missing required fields or unreadable documents -> Pending (specific, fixable)
    # 5. Field values mismatch between form and document -> Pending (quotes both values)
    # 6. Returning vendor with single expired document -> Pending (Lightweight)
    # 7. Bank account holder name doesn't clearly match company and no relationship note -> Pending
    # 8. Only soft, non-blocking signals remain -> Approved (with notes)
    # 9. Nothing flagged -> Approved
    ...
    if is_duplicate_bank_reuse:
        # Priority 1: Duplicate bank account reuse
    ...
    elif stage5.flag_type == "tax_country_fraud":
        # Priority 2: Tax ID country format fraud
        verdict = VerdictStatus.REJECTED
        ...
        # Priority 3: Uploaded registration/tax cert belongs to completely different company
        verdict = VerdictStatus.REJECTED
    ...
    elif stage1.status == StageStatus.FAIL or stage2.status == StageStatus.FAIL:
        # Priority 4: Missing fields or unreadable documents -> Fixable Pending
        ...
        # Priority 5: Form vs Document field mismatches (account number typo, reg number, etc.)
        verdict = VerdictStatus.PENDING
        ...
        # Priority 6: Returning vendor with expired document -> Lightweight Pending
        verdict = VerdictStatus.PENDING
        ...
        # Priorities 8 & 9: Approved (with notes if soft signals exist)
        verdict = VerdictStatus.APPROVED
        soft_signals = []
        if stage4.flag_type == "name_mismatch_explained":
    ...
    stage7 = StageResult(
        ...
        notes=[f"Verdict: {verdict.value}", f"Primary Rule Applied: {decision_flag or 'All Clean'}"]
    )
    ...
    yield {"event": "stage_update", "stage": stage7.model_dump(), "progress": 100}
